# Archived Job Postings

Job postings are ephemeral. Once a role is filled the page is taken down, and a link that supported an argument stops resolving. (One posting I had collected while researching this material, a DRW "Python Data Engineer" role, had already vanished from DRW's site before I finished writing.) To keep the evidence in [Why "Data Pipelines for Quantitative Research"?](./why_data_pipelines_for_quantitative_research.md) inspectable, each posting is preserved here on its own page with the full text, a screenshot, a PDF snapshot, and a link to a Wayback Machine copy. All were collected in August 2026.

```{toctree}
:maxdepth: 1
job_posting_jane_street.md
job_posting_citadel_securities.md
job_posting_imc.md
job_posting_drw.md
job_posting_point72.md
job_posting_jump_trading.md
job_posting_hudson_river_trading.md
job_posting_akuna_capital.md
job_posting_aqr.md
job_posting_jpmorganchase.md
```

- [Jane Street — Data Engineer](./job_posting_jane_street.md)
- [Citadel Securities — Senior Research Engineer (Data)](./job_posting_citadel_securities.md)
- [IMC Trading — Data Engineer](./job_posting_imc.md)
- [DRW — Data Engineer, Cumberland/FICCO](./job_posting_drw.md)
- [Point72 (Cubist Systematic Strategies) — Data Engineer](./job_posting_point72.md)
- [Jump Trading — Campus Data Engineer (Intern)](./job_posting_jump_trading.md)
- [Hudson River Trading — Data Production Engineer](./job_posting_hudson_river_trading.md)
- [Akuna Capital — Software Engineer, Data Engineering](./job_posting_akuna_capital.md)
- [AQR Capital Management — Portfolio Analytics Engineer](./job_posting_aqr.md)
- [JPMorganChase — Data & AI Internship Program](./job_posting_jpmorganchase.md)

Excerpts quoted from these postings are for educational commentary; full attribution and source links appear on each page.
